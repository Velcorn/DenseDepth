...
class AUTSL_BasicRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, shape_rgb, shape_depth):
        self.data = data
        self.dataset = dataset
        self.batch_size = batch_size
...
def get_autsl_data(batch_size, autsl_data_zipfile='autsl_data.zip'):
    data = extract_zip(autsl_data_zipfile)

    autsl_train = list(
        (row.split(',') for row in (data['data/autsl_train.csv']).decode("utf-8").split('\n') if len(row) > 0))
    autsl_test = list(
    ...
    return data, autsl_train, autsl_test, shape_rgb, shape_depth


def get_autsl_train_test_data(batch_size):
